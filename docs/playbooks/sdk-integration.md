...
- **T1 — SDK lifecycle.** Add the SDK as a dependency **pinned to a tag**. Resolve `LOG_DESTINATION` and the OTLP endpoint from env; the resolver **must never throw** — a bad/absent endpoint degrades silently, never blocks the app. If the target endpoint sits behind a write-auth proxy (any non-local endpoint usually does), resolve its auth token in the same step and thread it into `init`'s `headers` — an endpoint wired without its token compiles and runs but silently drops every write (see `docs/log-destinations.md`'s local/cloud endpoint section). Call the SDK `init` once at the **earliest** startup point, **gated on the destination including `grafana`**, idempotent. Wire flush/shutdown so the buffer drains before the process exits.
...
*Where* itself usually isn't a single value — most projects resolve between a local backend and a deployed cloud one, per environment, at build/deploy time. That selection (which file/mechanism holds it, the auth token that travels with a proxy-fronted endpoint, why real values never land in committed docs, and the CORS implication for a **deployed** browser build, not just its dev server) is generic across every platform and lives entirely in `docs/log-destinations.md`'s "Local vs cloud endpoint selection" section — read it before wiring T1's endpoint resolver.
...
- **CORS (browser).** Posting to Loki from a web app is cross-origin. Use a same-origin relative endpoint proxied to the backend (no preflight, and `sendBeacon` on unload works) rather than enabling CORS on the shared backend — the dev server's own proxy is only half of this; a **deployed** build needs the identical proxy rule at whatever real web server fronts it in that environment. Full treatment in `docs/log-destinations.md`.
- **A pinned SDK tag can lag behind the SDK's own `main`.** A feature visible in the SDK repo's source tree is not necessarily in the tag a consumer has pinned — check the actually pinned/locked ref (`package-lock.json`, `pubspec.lock`, `Package.resolved`), not the sibling repo's `HEAD`, before relying on a feature (a header option, a new param) existing. If it's on `main` but untagged, a new tag needs cutting and pushing before any consumer can pick it up — don't assume "it's in the code" means "it's available."
- **A branch ref is not a pin.** A dependency declared against a floating ref (e.g. a branch name) instead of a tag silently drifts onto whatever lands on that branch next — the opposite of the "pinned to a tag" principle (T1). If one is found, that's a standing bug to fix (point it at the current tag), not a style choice to leave alone.
