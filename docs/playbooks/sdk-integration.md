...
- **JS git-dependency builds.** A TypeScript SDK consumed by git URL must build its `dist/` on install — the package needs a `prepare` script, or a git install yields an empty package. npm caches git refs by tag, so after a tag is re-cut, clear the cache / lock entry before reinstalling.
